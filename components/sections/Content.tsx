...
export default function Content() {
	...
	return (
		<div className="max-w-4xl mx-auto space-y-8 pb-16 bg-gray-50">
			<header className="sticky top-0 bg-gradient-to-r from-blue-800 to-blue-600 text-white z-10 py-6 px-4 shadow-lg">
				...
				<div className="flex items-center space-x-2">
					<Input
						...
						className="flex-grow bg-white text-blue-900 placeholder-blue-400"
					/>
			...
			<Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
				<TabsList className="grid w-full grid-cols-4 bg-blue-100 rounded-lg p-1">
					{["all", "video", "audio", "text"].map((tab) => (
						<TabsTrigger
							key={tab}
							value={tab}
							className="text-blue-600 data-[state=active]:bg-white data-[state=active]:text-blue-700 rounded-md transition-all duration-200 ease-in-out">
							{t(`content.${tab}`)}
						</TabsTrigger>
					))}
				</TabsList>
					...
					<ScrollArea className="h-[calc(100vh-200px)]">
						...
						{filteredAndTypedItems.length > 0 && (
							<Button
								...
								className="w-full mt-4 bg-blue-600 hover:bg-blue-500 text-white transition-colors duration-200">
								{t("content.loadMore")}
							</Button>
						)}
					</ScrollArea>
